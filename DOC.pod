...
B<hexdens> v0.27 --  create density plots on a triangular or square lattice in postscript
or pdf format.
...
remainder of the data set.) If the data lines are of the form 'x y', then C<val> is assumed
to be C<1.0>
...
data is interpreted as "x y val". A line with two columns of data is interpreted
as "x y 1.0". A line with more than three numbers on a line is assumed to be
