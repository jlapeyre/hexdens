...
B<hexdens> v0.21a --  create density plots on a triangular or square lattice in postscript
or pdf format.
...
hexdens reads data from the input files specified on command
...
PDF. hexdens also operates in an interactive mode.
...
except that it is prepended with I<Hexdens::>, eg
I<$Hexdens::Title>.  To change plot parameters you may use
...
x\^ 2'> prints a Greek letter xsi with a superscript C<2> as
...
F<.hexdens>. You can set the environment variable GS_FONTPATH of the shell in which
you run hexdens to use fonts than that are not in the default path
of ghostscript. There is a $FontPath parameter in F<.hexdens> that does
...
=item C<\h+ > C<\h+2 >  '\h- ' '\h-2 '  '\v+ ' '\v+2 '  '\v- ' '\v-2 '
...
can be generated using B<-ex> I<n>. For instance I<hexdens -ex 2>
creates the second example plot. You can see the example plot
immediately if you use I<hexdens -display -ex 2>.

Using I<hexdens -demo>, produces all the examples sequentially and launhces the viewer to view them,
prompting the user after each plot. I<hexdens -demo N> instead pauses for N seconds between example
plots.
...
with 'no' when passing command line options to hexdens from a shell
...
mode using the B<set> command. In this case the leading '-' may
...
Hexdens requires a standard perl installation to produce postscript.

ghostscript ('gs') must  be installed to produce eps and pdf files.

ghostview ('gv') or xpdf must be installed if you want
hexdens to launch viewers at the appropriate time.  It is
