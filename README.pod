...
usage: tridens [options] [F<infilename1>, F<infilename2> ...]

=head1 DESCRIPTION

tridens reads data from the input files specified on command
line and, for each one, writes color or grayscale density
plots.  The input data is assumed to be on a rectangular or
triangular lattice. For the triangular(square) lattice, each
coordinate pair in the input file is covered with a
hexagon(square). Other data formats and plotting styles are
available as well. The output file format is PostScript or
PDF. tridens also operates in an interactive mode.
...
C<tridens -i data.dat> -- write a density plot and display the result in the
...
Here is an example:
 ...
 myshellprompt > tridens -i sq1.dat
 tridens> plot
...
data is used. If you have set the option B<-dis> then tridens will try to display the plot
...
When starting tridens from the command line boolean options
are unset by prefixing the option name with 'no'.  For
example: C<set cmgray title 'title of plot'>, sets the title
and sets grayscale mode.  On the commadline C<-nokey>
...
For example if the color map is set to 'cmhue' rather than 'cmgray':
...
Save the current plotting options to the file I<configfile> or 'save.tridens' if I<configfile> is omitted.
There is no 'save' command for the plot itself, as it always written to a file. This saves options
that are currently set. When starting a new tridens session, the command B<load> will restore
the saved parameter settings.

=item B<load> I<configfile>

Load saved plotting options from the file I<configfile> or 'save.tridens' if I<configfile> is omitted.
If the default filename for saved parameters 'save.tridens' is found when tridens starts, then
...
An attempt is made to detect one of the supported formats automatically
...
details described below) is set (or autodata detection is
set)  then the data file must consist
of only the values, with the coordinates implied by the row and column of
the number. With the grided format an n by m plot will be produced from a data file with m lines
...
'-noautolength' or by giving the option '-sidelength', in
...
This changes the text color. To see the list of color names, type C<tridens -listcolors n>,
or C<listcolors n> in interactive mode. If you omit the 'n',
...
with 'no' when passing command line options to tridens from a shell
...
mode using the 'set' command. In this case the leading '-' may
be omitted. Any options below that take an
...
B<-mandoc> 
print documentation as man page to file tridens.1

B<-htmldoc> 
print documentation to  tridens.html
...
B<-paramfile> 
write parameter startup file template to the file .tridens
...
B<-square>
(default) Cause squares to be drawn at data coordinates, not hexagons. By default
...
B<-tri>
assume a triangular lattice, I<ie> hexagons are drawn. This option automatically
sets the B<-sameunits> option.

B<-circle>
draw circles on data coordinates.  This option automatically
sets the B<-sameunits> option.
...
set boundaries of plot area, overriding automatic boundaries.
...
B<-power> I<POWER> 
(default value .5) power to use with -usepower
...
For instance set xlabel '$e^{x^2}$' will give the desired result. The numbers in the
...
set xlabel ' \white .................\black  $e^{x^2}$', to put a series of invisible
dots to acheive centering. Or try the option B<-xlabelxoffset>. Backslashes in
latex code must be escaped with a backslash. Ie. $\\frac{1}{2}$
...
the purpose of bbpad. These programs will  compute the bounding box around this white
frame. Otherwise small errors can cause a small part of the plot or text to
be clipped.
...
if B<-grided> is set. For instance, if the data is intended to be on a grid that is
...
B<-version>  
print tridens version number and exit.

B<-test>  
run the test suite. requires first running B<-generatetest> with a good copy of tridens.

B<-generatetest> 
generate the test suite. only for development. Currently broken
...
ghostview ('gv') or xpdf must be installed if you want
tridens to launch viewers at the appropriate time.  It is
probably possible to use other viewers as well.
...
be enabled. This makes the interactive mode much more friendly. The
perl gnu readline is probably best.

The ipe program is necessary for the B<-runlatex> option to process latex
text.
...
2) parameters in F<.tridens>, 3) parameters in F<infilename.p>,
4) parameters in F<save.tridens> (saved with the B<save> command),
5) parameters
...
On exiting tridens normally the interactive command history is automatically saved
(by gnu readline). The history will be read the next time tridens is started so that
the commands from the previous session are available in the history.

=head1 TROUBLESHOOTING

If tridens won't start or misbehaves, try deleting parameter files 'save.tridens'
and  '.tridens' as they may be out of date.
